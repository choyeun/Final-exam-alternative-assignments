    ...
    if ((N == 'Y') || (N == 'y')) {
        ...
        while (1) {
            ...
            while (1) {
                ...
                gotoxy(12, 5);
                scanf("%9s", sid);  // 필드 폭 지정자를 추가하여 문제 해결
                if (strlen(sid) > 0) break;
            }
            if (strcmp(sid, ".") == 0) { break; }    // 학번이 '.' 인지 확인 

            gotoxy(2, 6);    printf("-. 이름 : ");   scanf("%9s", name);  // 필드 폭 지정자를 추가하여 문제 해결
...
int ViewData() {
    ...
    while (!feof(fp1)) {
        fscanf(fp1, "%9s %9s %d %d %d", sid, name, &score1, &score2, &score3);  // 필드 폭 지정자를 추가하여 문제 해결
